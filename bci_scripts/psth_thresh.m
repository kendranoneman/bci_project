...
for i = 1:length(unique_sessidx)
    ...
    saveas(a,sprintf('psth/%s/thres/psth-correct-s%d.png',monkey,s));
    ...
    ylim([0 20]);
    legend('BCI','Block Sham','Location','Best');
    saveas(b,sprintf('psth/%s/thres/psth-error-s%d.png',monkey,s));
end
...
yline(0,'--k')
ylim([5 9])
%title(sprintf('Average Distance Normalized by Threshold Over Correct Trials Across Sessions vs. Time: %s',monkey));
xlabel('Time from BCI start (ms)','FontSize',fontsize);
ylabel('Avg. Distance/Threshold','FontSize',fontsize);
legend('BCI','Block Sham','Location','northeast');
saveas(t,sprintf('psth/%s/thres/psth-correct-all.png',monkey));
...
yline(0,'--k')
ylim([5 9])
%title(sprintf('Average Distance Normalized by Threshold Over Error Trials Across Sessions vs. Time: %s',monkey));
xlabel('Time from BCI start (ms)','FontSize',fontsize);
ylabel('Avg. Distance/Threshold','FontSize',fontsize);
legend('BCI','Block Sham','Location','northeast');
saveas(t,sprintf('psth/%s/thres/psth-error-all.png',monkey));    
...
saveas(tt,sprintf('psth/%s/thres/psth-diff-start-lines.png',monkey));
...
title(sprintf('Difference in Avg. Distance Normalized by Threshold (Block Sham - BCI) Across Sessions: %s', monkey));
saveas(tt2,sprintf('psth/%s/thres/psth-diff-start-hist.png',monkey));